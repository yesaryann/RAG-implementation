...
var GenerativeAgent = class extends require_base.BaseChain {
	...
	*/
	async getEntityFromObservations(observation, runManager) {
		const prompt = _langchain_core_prompts.PromptTemplate.fromTemplate("What is the observed entity in the following observation? {observation}\nEntity=");
		return (await this.chain(prompt).call({ observation }, runManager?.getChild("entity_extractor"))).output;
	}
	/**
	...
	*/
	async getEntityAction(observation, entityName, runManager) {
		const prompt = _langchain_core_prompts.PromptTemplate.fromTemplate("What is the {entity} doing in the following observation? {observation}\nThe {entity} is");
		return (await this.chain(prompt).call({
			entity: entityName,
			observation
		}, runManager?.getChild("entity_action_extractor"))).output.trim();
	}
	/**
	...
	*/
	async summarizeRelatedMemories(observation, runManager) {
		const prompt = _langchain_core_prompts.PromptTemplate.fromTemplate(`
{q1}?
Context from memory:
{relevant_memories}
Relevant context:`);
		...
		return (await this.chain(prompt).call({
			q1,
			queries: [q1, q2]
		}, runManager?.getChild("entity_relationships"))).output.trim();
	}
	async _call(values, runManager) {
		const { observation, suffix, now } = values;
		const prompt = _langchain_core_prompts.PromptTemplate.fromTemplate(`{agent_summary_description}
It is {current_time}.
...
Observation: {observation}\n\n${suffix}`);
		...
		const chainInputs = {
			...
			current_time: (now || /* @__PURE__ */ new Date()).toLocaleString("en-US", {
				month: "long",
				day: "numeric",
				year: "numeric",
				hour: "numeric",
		...
		const rawOutput = (await this.chain(prompt).call(chainInputs, runManager?.getChild("reaction_from_summary"))).output;
	...
	*/
	async generateReaction(observation, now) {
		const { output, continue_dialogue } = await this.call({
			observation,
			suffix: "Should {agent_name} react to the observation, and if so, what would be an appropriate reaction? Respond in one line. If the action is to engage in dialogue, write:\nSAY: \"what to say\" \notherwise, write:\nREACT: {agent_name}'s reaction (if anything). \nEither do nothing, react, or say something but not both.\n\n",
			now
		});
...
${this.summary}`;
	...
	*/
	async computeAgentSummary(runManager) {
		const prompt = _langchain_core_prompts.PromptTemplate.fromTemplate("How would you summarize {name}'s core characteristics given the following statements:\n----------{relevant_memories}----------Do not embellish.\n\nSummary: ");
		return (await this.chain(prompt).call({
			name: this.name,
			queries: [`${this.name}'s core characteristics`]
		}, runManager?.getChild("compute_agent_summary"))).output.trim();
	}
	/**
	...
	*/
	getFullHeader(config = {}) {
		...
		return `${this.getSummary({
			now,
			forceRefresh
		})}\nIt is ${now.toLocaleString("en-US", {
			month: "long",
			day: "numeric",
			year: "numeric",
			hour: "numeric",
