...
ORDER BY TABLE_NAME, ORDINAL_POSITION;`;
	...
	if (appDataSource.options.type === "sap") {
      ...
      FROM TABLE_COLUMNS
      WHERE SCHEMA_NAME='${appDataSource.options?.schema ?? "public"}'`;
		const rep = await appDataSource.query(sql);
		const repLowerCase = [];
	...
	if (appDataSource.options.type === "oracle") {
      ...
      FROM ALL_TAB_COLS
      WHERE
          OWNER = UPPER('${appDataSource.options.schema}')`;
		const rep = await appDataSource.query(sql);
		return formatToSqlTable(rep);
	}
...
const generateTableInfoFromTables = async (tables, appDataSource, nbSampleRow, customDescription) => {
	...
	for (const currentTable of tables) {
		...
		else if (appDataSource.options.type === "postgres") sqlSelectInfoQuery = `SELECT * FROM "${appDataSource.options.type === "postgres" ? appDataSource.options?.schema ?? "public" : "public"}"."${currentTable.tableName}" LIMIT ${nbSampleRow};\n`;
		else if (appDataSource.options.type === "mssql") {
			const schema = appDataSource.options?.schema;
			sqlSelectInfoQuery = schema ? `SELECT TOP ${nbSampleRow} * FROM ${schema}.[${currentTable.tableName}];\n` : `SELECT TOP ${nbSampleRow} * FROM [${currentTable.tableName}];\n`;
		} else if (appDataSource.options.type === "sap") sqlSelectInfoQuery = `SELECT * FROM "${appDataSource.options?.schema ?? appDataSource.options?.username ?? "public"}"."${currentTable.tableName}" LIMIT ${nbSampleRow};\n`;
