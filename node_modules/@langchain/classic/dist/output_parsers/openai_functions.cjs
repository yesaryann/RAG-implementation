...
let _langchain_core_utils_json_patch = require("@langchain/core/utils/json_patch");
//#region src/output_parsers/openai_functions.ts
/**
* Class for parsing the output of an LLM. Can be configured to return
* only the arguments of the function call in the output.
*/
var OutputFunctionsParser = class extends _langchain_core_output_parsers.BaseLLMOutputParser {
	static lc_name() {
...
*/
var JsonOutputFunctionsParser = class extends _langchain_core_output_parsers.BaseCumulativeTransformOutputParser {
	static lc_name() {
	...
	_diff(prev, next) {
		if (!next) return;
		return (0, _langchain_core_utils_json_patch.compare)(prev ?? {}, next);
	}
	async parsePartialResult(generations) {
		...
		if (this.argsOnly) return (0, _langchain_core_output_parsers.parsePartialJson)(functionCall.arguments);
		return {
			...functionCall,
			arguments: (0, _langchain_core_output_parsers.parsePartialJson)(functionCall.arguments)
		};
	}
	/**
	...
	*/
	...
	async parse(text) {
		try {
			...
			throw new _langchain_core_output_parsers.OutputParserException(`Failed to parse. Text: "${text}". Error: ${e}`);
		}
	}
...
* to parse the output.
*/
var JsonKeyOutputFunctionsParser = class extends _langchain_core_output_parsers.BaseLLMOutputParser {
	static lc_name() {
