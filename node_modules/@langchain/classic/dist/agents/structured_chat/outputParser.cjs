...
var StructuredChatOutputParser = class extends require_types.AgentActionOutputParser {
	...
	*/
	async parse(text) {
		try {
			...
			if (actionMatch === null) throw new _langchain_core_output_parsers.OutputParserException(`Could not parse an action. The agent action must be within a markdown code block, and "action" must be a provided tool or "Final Answer"`);
	...
	*/
	getFormatInstructions() {
		return (0, _langchain_core_prompts.renderTemplate)(require_prompt.AGENT_ACTION_FORMAT_INSTRUCTIONS, "f-string", { tool_names: this.toolNames.join(", ") });
	}
};
/**
