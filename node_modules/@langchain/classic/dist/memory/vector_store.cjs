...
* ```
*/
var VectorStoreRetrieverMemory = class extends _langchain_core_memory.BaseMemory {
	...
	*/
	async loadMemoryVariables(values) {
		const query = (0, _langchain_core_memory.getInputValue)(values, this.inputKey);
		const results = await this.vectorStoreRetriever.invoke(query);
		return { [this.memoryKey]: this.returnDocs ? results : require_util_document.formatDocumentsAsString(results) };
	}
	/**
	...
	*/
	async saveContext(inputValues, outputValues) {
		...
		await this.vectorStoreRetriever.addDocuments([new _langchain_core_documents.Document({
			pageContent: text,
			metadata
		})]);
	}
...
//# sourceMappingURL=vector_store.cjs.map