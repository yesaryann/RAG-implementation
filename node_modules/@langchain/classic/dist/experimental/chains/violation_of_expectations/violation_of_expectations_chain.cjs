...
*/
var ViolationOfExpectationsChain = class extends require_base.BaseChain {
	static lc_name() {
	...
	*/
	async getPredictionViolations({ userPredictions, userResponse, runManager }) {
		const llmWithFunctions = this.llm.bindTools([require_types.PREDICTION_VIOLATIONS_FUNCTION]).withConfig({ function_call: { name: require_types.PREDICTION_VIOLATIONS_FUNCTION.name } });
		const chain = require_violation_of_expectations_prompt.PREDICTION_VIOLATIONS_PROMPT.pipe(llmWithFunctions).pipe(this.jsonOutputParser);
...
//# sourceMappingURL=violation_of_expectations_chain.cjs.map