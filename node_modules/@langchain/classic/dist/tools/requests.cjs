...
*/
var RequestsGetTool = class extends _langchain_core_tools.Tool {
	headers;
	static lc_name() {
	...
	/** @ignore */
	async _call(input) {
		return (await (await fetch(input, { headers: this.headers })).text()).slice(0, this.maxOutputLength);
	}
...
* POST request.
*/
var RequestsPostTool = class extends _langchain_core_tools.Tool {
	headers;
	static lc_name() {
	...
	async _call(input) {
		try {
			...
			})).text()).slice(0, this.maxOutputLength);
		} catch (error) {
			return `${error}`;
		}
	}
