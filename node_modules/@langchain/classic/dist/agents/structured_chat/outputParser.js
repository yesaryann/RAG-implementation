...
var StructuredChatOutputParser = class extends AgentActionOutputParser {
	...
	*/
	async parse(text) {
		try {
			const actionMatch = /```(?:json)?(.*)(```)/gs.exec(text);
			if (actionMatch === null) throw new OutputParserException(`Could not parse an action. The agent action must be within a markdown code block, and "action" must be a provided tool or "Final Answer"`);
			const { action, action_input } = JSON.parse(actionMatch[1].trim());
	...
	*/
	getFormatInstructions() {
		return renderTemplate(AGENT_ACTION_FORMAT_INSTRUCTIONS, "f-string", { tool_names: this.toolNames.join(", ") });
	}
};
/**
...
* ```
*/
var StructuredChatOutputParserWithRetries = class StructuredChatOutputParserWithRetries extends AgentActionOutputParser {
	lc_namespace = [
		"langchain",
		"agents",
		"structured_chat"
...
//#endregion
export { StructuredChatOutputParser, StructuredChatOutputParserWithRetries };
