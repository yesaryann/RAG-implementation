	...
	if (message.tool_calls !== void 0 && message.tool_calls.length > 0) toolCalls = message.tool_calls;
		...
		for (const toolCall of message.additional_kwargs.tool_calls ?? []) {
			...
			try {
				...
				throw new _langchain_core_output_parsers.OutputParserException(`Failed to parse tool arguments from chat model response. Text: "${JSON.stringify(toolCalls)}". ${e}`);
			}
		}
	}
...
var ToolCallingAgentOutputParser = class extends require_types.AgentMultiActionOutputParser {
	...
	async parseResult(generations) {
		if ("message" in generations[0] && (0, _langchain_core_messages.isBaseMessage)(generations[0].message)) return parseAIMessageToToolAction(generations[0].message);
