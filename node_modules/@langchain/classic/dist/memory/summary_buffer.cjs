...
var ConversationSummaryBufferMemory = class extends require_summary.BaseConversationSummaryMemory {
	...
	*/
	async loadMemoryVariables(_) {
		...
		else finalBuffer = (0, _langchain_core_messages.getBufferString)(buffer, this.humanPrefix, this.aiPrefix);
	...
	*/
	async prune() {
		let buffer = await this.chatHistory.getMessages();
		if (this.movingSummaryBuffer) buffer = [new this.summaryChatMessageClass(this.movingSummaryBuffer), ...buffer];
		let currBufferLength = await this.llm.getNumTokens((0, _langchain_core_messages.getBufferString)(buffer, this.humanPrefix, this.aiPrefix));
		if (currBufferLength > this.maxTokenLimit) {
			...
			while (currBufferLength > this.maxTokenLimit) {
				...
				if (poppedMessage) {
					...
					currBufferLength = await this.llm.getNumTokens((0, _langchain_core_messages.getBufferString)(buffer, this.humanPrefix, this.aiPrefix));
				}
			}
