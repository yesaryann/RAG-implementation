...
function _getLanguageModel(llmLike) {
	...
	else if ("bound" in llmLike && _langchain_core_runnables.Runnable.isRunnable(llmLike.bound)) return _getLanguageModel(llmLike.bound);
	else if ("runnable" in llmLike && "fallbacks" in llmLike && _langchain_core_runnables.Runnable.isRunnable(llmLike.runnable)) return _getLanguageModel(llmLike.runnable);
	else if ("default" in llmLike && _langchain_core_runnables.Runnable.isRunnable(llmLike.default)) return _getLanguageModel(llmLike.default);
...
var LLMChain = class LLMChain extends require_base.BaseChain {
	...
	*/
	...
	async _call(values, runManager) {
		...
		const response = await (this.outputParser ? this.llm.pipe(this.outputParser) : this.llm).invoke(promptValue, runManager?.getChild());
	...
	*/
	async predict(values, callbackManager) {
		return (await this.call(values, callbackManager))[this.outputKey];
	}
	_chainType() {
		return "llm";
	}
	static async deserialize(data) {
		...
		return new LLMChain({
			llm: await _langchain_core_language_models_base.BaseLanguageModel.deserialize(llm),
			prompt: await _langchain_core_prompts.BasePromptTemplate.deserialize(prompt)
		});
	}
	/** @deprecated */
	serialize() {
