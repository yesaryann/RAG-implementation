...
const convertResponsesUsageToUsageMetadata = (usage) => {
	const inputTokenDetails = {
		...usage?.input_tokens_details?.cached_tokens != null && { cache_read: usage?.input_tokens_details?.cached_tokens },
		...usage?.input_tokens_details?.cache_write_tokens != null && { cache_creation: usage?.input_tokens_details?.cache_write_tokens }
	};
 ...
 /**
		...
		if (role === "assistant") {
			...
			*/
			...
			if (_langchain_core_messages.AIMessage.isInstance(lcMsg) && !!lcMsg.tool_calls?.length) input.push(...lcMsg.tool_calls.map((toolCall) => {
				if (require_tools.isCustomToolCall(toolCall, customToolCallIds)) return {
		...
		if (role === "user" || role === "system" || role === "developer") {
			...
			const content = lcMsg.content.flatMap((item) => {
				...
				if ((0, _langchain_core_messages.isDataContentBlock)(item)) {
					if (item.type === "file") {
						const filename = require_misc.getFilenameFromMetadata(item);
