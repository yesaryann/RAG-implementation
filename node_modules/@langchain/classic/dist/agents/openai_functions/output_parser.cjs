...
var OpenAIFunctionsAgentOutputParser = class extends require_types.AgentActionOutputParser {
	...
	async parseResult(generations) {
		if ("message" in generations[0] && (0, _langchain_core_messages.isBaseMessage)(generations[0].message)) return this.parseAIMessage(generations[0].message);
	...
	parseAIMessage(message) {
		...
		if (message.additional_kwargs.function_call) {
			...
			try {
				...
				throw new _langchain_core_output_parsers.OutputParserException(`Failed to parse function arguments from chat model response. Text: "${function_call.arguments}". ${error}`);
			}
		} else return {
