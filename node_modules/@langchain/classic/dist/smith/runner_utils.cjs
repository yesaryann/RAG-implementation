...
let _langchain_core_tracers_base = require("@langchain/core/tracers/base");
let _langchain_core_tracers_tracer_langchain = require("@langchain/core/tracers/tracer_langchain");
...
var SingleRunExtractor = class extends _langchain_core_tracers_base.BaseTracer {
	runPromiseResolver;
	runPromise;
...
var DynamicRunEvaluator = class {
	...
	constructor(evaluator) {
		this.evaluator = new _langchain_core_runnables.RunnableLambda({ func: evaluator });
	}
	/**
	...
	*/
	async evaluateRun(run, example) {
		...
		const tracer = new _langchain_core_tracers_tracer_langchain.LangChainTracer({ projectName: "evaluators" });
		...
		return {
			sourceRunId: await extractor.extract(),
			...result
		};
	}
};
...
var RunnableTraceable = class extends _langchain_core_runnables.Runnable {
	...
	async invoke(input, options) {
		...
		const callbackManager = await (0, _langchain_core_runnables.getCallbackManagerForConfig)(config);
...
const createWrappedModel = async (modelOrFactory) => {
	if (_langchain_core_runnables.Runnable.isRunnable(modelOrFactory)) return () => modelOrFactory;
	if (typeof modelOrFactory === "function") {
		...
		try {
			...
			const wrappedModel = new _langchain_core_runnables.RunnableLambda({ func: modelOrFactory });
			return () => wrappedModel;
		}
	}
...
const loadExamples = async ({ datasetName, client, projectName }) => {
	...
	for await (const example of exampleIterator) {
		...
		configs.push({ callbacks: [new _langchain_core_tracers_tracer_langchain.LangChainTracer({
...
const applyEvaluators = async ({ evaluation, runs, examples, client, maxConcurrency }) => {
	...
	const caller = new _langchain_core_utils_async_caller.AsyncCaller({ maxConcurrency });
	...
	return (await Promise.all(requests)).reduce((acc, result, i) => ({
		...acc,
		[examples[i].id]: result
	}), {});
};
const getExamplesInputs = (examples, chainOrFactory, dataType) => {
	if (dataType === "chat") return examples.map(({ inputs }) => (0, _langchain_core_messages.mapStoredMessagesToChatMessages)(inputs.input));
	if (typeof chainOrFactory === "object" && typeof chainOrFactory._llmType === "function" && examples.every(({ inputs }) => Object.keys(inputs).length === 1)) return examples.map(({ inputs }) => Object.values(inputs)[0]);
...
async function runOnDataset(chainOrFactory, datasetName, options) {
	...
	const wrappedRunnable = new _langchain_core_runnables.RunnableLambda({ func: wrappedModel }).withConfig({ runName: "evaluationRun" });
