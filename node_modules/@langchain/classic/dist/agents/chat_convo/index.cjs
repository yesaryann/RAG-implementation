	...
	*/
	static createPrompt(tools, args) {
		...
		const renderedHumanMessage = (0, _langchain_core_prompts.renderTemplate)(humanMessage, "f-string", {
			format_instructions: (args?.outputParser ?? ChatConversationalAgent.getDefaultOutputParser({ toolNames })).getFormatInstructions({ toolNames }),
			tools: toolStrings
		});
		const messages = [
	...
	*/
	static fromLLMAndTools(llm, tools, args) {
		...
		const chain = new require_llm_chain.LLMChain({
			prompt: ChatConversationalAgent.createPrompt(tools, {
				...args,
				outputParser
			}),
			llm,
