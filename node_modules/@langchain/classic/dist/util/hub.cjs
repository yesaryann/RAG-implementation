...
const loadFromHub = async (uri, loader, validPrefix, validSuffixes, values = {}) => {
	const LANGCHAIN_HUB_DEFAULT_REF = (0, _langchain_core_utils_env.getEnvironmentVariable)("LANGCHAIN_HUB_DEFAULT_REF") ?? "master";
	const LANGCHAIN_HUB_URL_BASE = (0, _langchain_core_utils_env.getEnvironmentVariable)("LANGCHAIN_HUB_URL_BASE") ?? "https://raw.githubusercontent.com/hwchase17/langchain-hub/";
	...
	if (remotePath.split(URL_PATH_SEPARATOR)[0] !== validPrefix) return;
	...
	const res = await require_index.default(() => fetchWithTimeout(url, { timeout: 5e3 }), { retries: 6 });
