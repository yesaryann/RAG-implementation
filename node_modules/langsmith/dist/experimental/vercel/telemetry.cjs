...
const _agent_type_js_1 = require("./_agent_type.cjs");
...
function _hasNonzeroUsageMetadata(runTree) {
    const usageMetadata = runTree.extra?.metadata?.usage_metadata;
    if (!(0, types_js_1.isRecord)(usageMetadata))
        return false;
    return ["input_tokens", "output_tokens", "total_tokens"].some((key) => typeof usageMetadata[key] === "number" && usageMetadata[key] > 0);
}
/**
...
function LangSmithTelemetry(config) {
    ...
    function formatToolResultMessage(toolCallId, toolName, toolOutput) {
        const rawOutput = (0, types_js_1.isRecord)(toolOutput)
    ...
    function appendToolResultMessage(harnessState, toolCallId, toolName, toolOutput) {
    ...
    async function flushPiDeferredStepRun(state) {
    ...
    const onStart = async (event) => {
        if (!(0, env_js_1.isEnvTracingEnabled)(tracingEnabled))
            return;
        ...
        const runTreeConfig = {
            ...
            extra: {
                ...
                metadata: {
                    ...
                    ...(0, _agent_type_js_1.vercelLsAgentTypeMetadata)(parentRunTree),
        ...
        invocationsByCallId.set(event.callId, {
            ...
            experimental_harness: event.operationId === "ai.harness"
                ? {
                    lastStepContent: [],
                    completedStepMessages: [],
                    toolResults: new Map(),
                    piAggregateUsageWorkaround: event.provider === "pi" ? {} : undefined,
                }
                : undefined,
        });
    };
    const onStepStart = async (event) => {
        ...
        const piUsageWorkaround = state.experimental_harness?.piAggregateUsageWorkaround;
    ...
    const onToolExecutionStart = async (event) => {
        ...
        if (event.recordInputs !== false) {
            ...
            if ((0, types_js_1.isRecord)(toolCallInput)) {
    ...
    const onToolExecutionEnd = async (event) => {
        ...
        if (event.recordOutputs !== false) {
            ...
            else if ((0, types_js_1.isRecord)(event.toolOutput) && "error" in event.toolOutput) {
    ...
    const onStepFinish = async (event) => {
        ...
        if (harnessState != null && Array.isArray(event.content)) {
            harnessState.completedStepMessages.push((0, utils_js_1.convertMessageToTracedFormat)({
                role: "assistant",
                content: event.content,
            }));
            for (const part of event.content) {
                if (!(0, types_js_1.isRecord)(part) ||
                    part.type !== "tool-call" ||
                ...
                if (toolResult) {
                    appendToolResultMessage(harnessState, part.toolCallId, toolResult.toolName, toolResult.output);
    ...
    const onEnd = async (event) => {
        ...
        // Pi reports zero usage on inferred step boundaries and provides the actual
