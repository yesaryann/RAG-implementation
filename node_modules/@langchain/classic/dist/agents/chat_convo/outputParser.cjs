	...
	*/
	async parse(text) {
		...
		if (jsonOutput.includes("```json") || jsonOutput.includes("```")) {
			...
			if (jsonOutput.indexOf("action_input") > firstIndex) {
		...
		try {
			const { action, action_input } = JSON.parse(jsonOutput);
...
var ChatConversationalAgentOutputParserWithRetries = class ChatConversationalAgentOutputParserWithRetries extends require_types.AgentActionOutputParser {
	...
	*/
	getFormatInstructions(options) {
		...
		return (0, _langchain_core_prompts.renderTemplate)(require_prompt.FORMAT_INSTRUCTIONS, "f-string", { tool_names: options.toolNames.join(", ") });
	}
	/**
