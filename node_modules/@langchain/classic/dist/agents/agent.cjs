...
*/
var BaseAgent = class extends _langchain_core_load_serializable.Serializable {
	get returnValues() {
		return ["output"];
	}
	get allowedTools() {}
...
var AgentRunnableSequence = class extends _langchain_core_runnables.RunnableSequence {
	streamRunnable;
	singleAction;
	static fromRunnables([first, ...runnables], config) {
		const sequence = _langchain_core_runnables.RunnableSequence.from([first, ...runnables], config.name);
...
var RunnableSingleActionAgent = class extends BaseSingleActionAgent {
	...
	*/
	...
	async plan(steps, inputs, callbackManager, config) {
		...
		const combinedConfig = (0, _langchain_core_runnables.patchConfig)(config, {
	...
	static async deserialize(data) {
		switch (data._type) {
			case "zero-shot-react-description": {
				const { ZeroShotAgent } = await Promise.resolve().then(() => require("./mrkl/index.cjs"));
