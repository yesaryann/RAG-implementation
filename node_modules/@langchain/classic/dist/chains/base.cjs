...
*/
var BaseChain = class extends _langchain_core_language_models_base.BaseLangChain {
	get lc_namespace() {
		return [
			"langchain",
			"chains",
	...
	*/
	async invoke(input, options) {
		...
		const runManager = await (await _langchain_core_callbacks_manager.CallbackManager.configure(config?.callbacks, this.callbacks, config?.tags, this.tags, config?.metadata, this.metadata, { verbose: this.verbose }))?.handleChainStart(this.toJSON(), fullValues, void 0, void 0, void 0, void 0, config?.runName);
		let outputValues;
		try {
		...
		Object.defineProperty(outputValues, _langchain_core_outputs.RUN_KEY, {
	...
	*/
	...
	async run(input, config) {
		...
		if (!(inputKeys.length <= 1)) throw new Error(`Chain ${this._chainType()} expects multiple inputs, cannot use 'run' `);
	...
	*/
	async call(values, config, tags) {
		const parsedConfig = {
			tags,
			...(0, _langchain_core_callbacks_manager.parseCallbackConfigArg)(config)
		};
	...
	*/
	static async deserialize(data, values = {}) {
		switch (data._type) {
			case "llm_chain": {
				const { LLMChain } = await Promise.resolve().then(() => require("./llm_chain.cjs"));
				return LLMChain.deserialize(data);
			}
			case "sequential_chain": {
				const { SequentialChain } = await Promise.resolve().then(() => require("./sequential_chain.cjs"));
				return SequentialChain.deserialize(data);
			}
			case "simple_sequential_chain": {
				const { SimpleSequentialChain } = await Promise.resolve().then(() => require("./sequential_chain.cjs"));
				return SimpleSequentialChain.deserialize(data);
			}
			case "stuff_documents_chain": {
				const { StuffDocumentsChain } = await Promise.resolve().then(() => require("./combine_docs_chain.cjs"));
				return StuffDocumentsChain.deserialize(data);
			}
			case "map_reduce_documents_chain": {
				const { MapReduceDocumentsChain } = await Promise.resolve().then(() => require("./combine_docs_chain.cjs"));
				return MapReduceDocumentsChain.deserialize(data);
			}
			case "refine_documents_chain": {
				const { RefineDocumentsChain } = await Promise.resolve().then(() => require("./combine_docs_chain.cjs"));
				return RefineDocumentsChain.deserialize(data);
			}
			case "vector_db_qa": {
				const { VectorDBQAChain } = await Promise.resolve().then(() => require("./vector_db_qa.cjs"));
				return VectorDBQAChain.deserialize(data, values);
			}
			case "api_chain": {
				const { APIChain } = await Promise.resolve().then(() => require("./api/api_chain.cjs"));
