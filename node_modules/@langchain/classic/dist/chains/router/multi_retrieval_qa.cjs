...
var MultiRetrievalQAChain = class MultiRetrievalQAChain extends require_multi_route.MultiRouteChain {
	...
	*/
	static fromLLMAndRetrievers(llm, { retrieverNames, retrieverDescriptions, retrievers, retrieverPrompts, defaults, multiRetrievalChainOpts, retrievalQAChainOpts }) {
		...
		const outputParser = new require_router.RouterOutputParser(zod_v3.z.object({
		...
		const routerPrompt = new _langchain_core_prompts.PromptTemplate({
			template: (0, _langchain_core_prompts.interpolateFString)(require_multi_retrieval_prompt.STRUCTURED_MULTI_RETRIEVAL_ROUTER_TEMPLATE(outputParser.getFormatInstructions({ interpolationDepth: 4 })), { destinations: destinationsStr }),
			inputVariables: ["input"],
			outputParser
		});
		const routerChain = require_llm_router.LLMRouterChain.fromLLM(llm, routerPrompt);
		const destinationChains = require_utils.zipEntries(retrieverNames, retrievers, retrieverPrompts ?? retrievers.map(() => null)).reduce((acc, [name, retriever, prompt]) => {
		...
		else _defaultChain = new require_conversation.ConversationChain({
			llm,
			prompt: new _langchain_core_prompts.PromptTemplate({
				template: require_conversation.DEFAULT_TEMPLATE.replace("input", "query"),
