...
*/
var StuffDocumentsChain = class StuffDocumentsChain extends BaseChain {
	static lc_name() {
	...
	_prepInputs(values) {
		...
		const text = docs.map(({ pageContent }) => pageContent).join("\n\n");
		return {
			...rest,
	...
	async _call(values, runManager) {
		return await this.llmChain.call(this._prepInputs(values), runManager?.getChild("combine_documents"));
	}
	_chainType() {
	...
	static async deserialize(data) {
		...
		return new StuffDocumentsChain({ llmChain: await LLMChain.deserialize(data.llm_chain) });
	}
	serialize() {
		return {
...
* @augments StuffDocumentsChainInput
*/
var MapReduceDocumentsChain = class MapReduceDocumentsChain extends BaseChain {
	static lc_name() {
	...
	async _call(values, runManager) {
		...
		for (let i = 0; i < this.maxIterations; i += 1) {
			...
			if (i !== 0 || !this.ensureMapStep) {
				const formatted = await this.combineDocumentChain.llmChain.prompt.format(this.combineDocumentChain._prepInputs({
					[this.combineDocumentChain.inputKey]: currentDocs,
					...rest
				}));
				if (await this.combineDocumentChain.llmChain._getNumTokens(formatted) < this.maxTokens) break;
			}
			const results = await this.llmChain.apply(inputs, runManager ? Array.from({ length: inputs.length }, (_, i) => runManager.getChild(`map_${i + 1}`)) : void 0);
...
*/
var RefineDocumentsChain = class RefineDocumentsChain extends BaseChain {
	static lc_name() {
	...
	async _constructInitialInputs(doc, rest) {
		...
		return {
			[this.documentVariableName]: await this.documentPrompt.format({ ...documentInfo }),
			...rest
		};
	}
	/** @ignore */
	...
	async _call(values, runManager) {
		...
		for (let i = 1; i < currentDocs.length; i += 1) {
			const inputs = {
				...await this._constructRefineInputs(currentDocs[i], res),
				...rest
			};
