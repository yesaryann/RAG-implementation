...
var StructuredOutputParser = class extends _langchain_core_output_parsers.BaseOutputParser {
	schema;
	static lc_name() {
...
\`\`\`json
${JSON.stringify((0, _langchain_core_utils_json_schema.toJsonSchema)(this.schema))}
\`\`\`
`;
	}
	/**
	...
	*/
	async parse(text) {
		try {
			...
			return await (0, _langchain_core_utils_types.interopParseAsync)(this.schema, JSON.parse(json));
		} catch {
			try {
				return await (0, _langchain_core_utils_types.interopParseAsync)(this.schema, JSON.parse(text.trim()));
			} catch (e2) {
				throw new _langchain_core_output_parsers.OutputParserException(`Failed to parse. Text: "${text}". Error: ${e2}`, text);
			}
		}
	}
};
...
var JsonMarkdownStructuredOutputParser = class extends StructuredOutputParser {
	...
	getFormatInstructions(options) {
		...
		return `Return a markdown code snippet with a JSON object formatted to look like:\n\`\`\`json\n${this._schemaToInstruction((0, _langchain_core_utils_json_schema.toJsonSchema)(this.schema)).replaceAll("{", "{".repeat(interpolationDepth)).replaceAll("}", "}".repeat(interpolationDepth))}\n\`\`\``;
	}
...
* output schemas.
*/
var AsymmetricStructuredOutputParser = class extends _langchain_core_output_parsers.BaseOutputParser {
	...
	async parse(text) {
		...
		try {
			...
			throw new _langchain_core_output_parsers.OutputParserException(`Failed to parse. Text: "${text}". Error: ${e}`, text);
		}
