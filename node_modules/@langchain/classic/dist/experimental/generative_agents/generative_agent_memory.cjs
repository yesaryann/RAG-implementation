...
var GenerativeAgentMemoryChain = class GenerativeAgentMemoryChain extends require_base.BaseChain {
	...
	*/
	...
	async _call(values, runManager) {
		...
		const document = new _langchain_core_documents.Document({
	...
	*/
	async scoreMemoryImportance(memoryContent, runManager) {
		const prompt = _langchain_core_prompts.PromptTemplate.fromTemplate("On the scale of 1 to 10, where 1 is purely mundane (e.g., brushing teeth, making bed) and 10 is extremely poignant (e.g., a break up, college acceptance), rate the likely poignancy of the following piece of memory. Respond with a single integer.\nMemory: {memory_content}\nRating: ");
		const strippedScore = (await this.chain(prompt).run(memoryContent, runManager?.getChild("determine_importance"))).trim();
		if (this.verbose) console.log("Importance score:", strippedScore);
		const match = strippedScore.match(/^\D*(\d+)/);
		if (match) return parseFloat(match[1]) / 10 * this.importanceWeight;
		else return 0;
	}
	/**
	...
	*/
	async getTopicsOfReflection(lastK, runManager) {
		const prompt = _langchain_core_prompts.PromptTemplate.fromTemplate("{observations}\n\nGiven only the information above, what are the 3 most salient high-level questions we can answer about the subjects in the statements? Provide each question on a new line.\n\n");
		const observationStr = this.memoryRetriever.getMemoryStream().slice(-lastK).map((o) => o.pageContent).join("\n");
	...
	*/
	async getInsightsOnTopic(topic, now, runManager) {
		const prompt = _langchain_core_prompts.PromptTemplate.fromTemplate("Statements about {topic}\n{related_statements}\n\nWhat 5 high-level insights can you infer from the above statements? (example format: insight (because of 1, 5, 3))");
		const relatedStatements = (await this.fetchMemories(topic, now, runManager)).map((memory, index) => `${index + 1}. ${memory.pageContent}`).join("\n");
...
* ```
*/
var GenerativeAgentMemory = class extends _langchain_core_memory.BaseMemory {
	llm;
	memoryRetriever;
	verbose;
	reflectionThreshold;
	...
	*/
	formatMemoriesDetail(relevantMemories) {
		...
		return content.map((mem) => `${mem}`).join("\n");
	}
	/**
	...
	*/
	formatMemoriesSimple(relevantMemories) {
		return relevantMemories.map((mem) => `${mem.pageContent}`).join("; ");
	}
	/**
