...
*/
async function createStuffDocumentsChain({ llm, prompt, outputParser = new _langchain_core_output_parsers.StringOutputParser(), documentPrompt = require_base.DEFAULT_DOCUMENT_PROMPT, documentSeparator = "\n\n" }) {
	if (!prompt.inputVariables.includes("context")) throw new Error(`Prompt must include a "${require_base.DOCUMENTS_KEY}" variable`);
	return _langchain_core_runnables.RunnableSequence.from([
		_langchain_core_runnables.RunnablePassthrough.assign({ [require_base.DOCUMENTS_KEY]: new _langchain_core_runnables.RunnablePick(require_base.DOCUMENTS_KEY).pipe((documents, config) => require_base.formatDocuments({
			documents,
			documentPrompt,
			documentSeparator,
			config
