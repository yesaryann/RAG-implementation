...
const convertStandardContentMessageToCompletionsMessage = ({ message, model }) => {
	...
	if (role === "developer") return {
		...
		if (Array.isArray(completionParam.content) && completionParam.content.length === 0 && completionParam.tool_calls != null) completionParam.content = null;
...
const convertMessagesToCompletionsMessageParams = ({ messages, model }) => {
	return messages.flatMap((message) => {
		...
		const content = typeof message.content === "string" ? message.content : message.content.flatMap((m) => {
			...
			if (typeof m === "object" && m !== null && "type" in m && (m.type === "tool_use" || m.type === "tool_call" || m.type === "functionCall" || m.type === "reasoning" || m.type === "reasoning_content" || m.type === "thinking")) return [];
			return m;
		});
		const completionParam = {
			role,
